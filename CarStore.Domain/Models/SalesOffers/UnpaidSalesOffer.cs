...
namespace CarStore.Domain.Models.SalesOffers
{
        ...
        where T : notnull
    {
        ...
        public UnpaidSalesOffer(T id, User<T> author, Car<T> car, Address address, Phone phone, Email email, Description description, Price price, Price priceOfPublishOffer, Title title)
            : base(id)
        {
            Author = author;
            Car = car;
        ...
        public SalesOffer<T> PayForPublicationOffer(Price price)
        {
            ...
            SalesOffer<T> salesOffer = new(Id, Author, Car, Address, Phone, Email, Description, Price, Title);
            return salesOffer;
        }
    }
}